...
#include "CooldownChanged.h"

UCooldownChanged* UCooldownChanged::ListenForCooldownChange(UAbilitySystemComponent* AbilitySystemComponent,
                                                            FGameplayTagContainer InCooldownTags,
                                                            bool InUseServerCooldown)
{
	UCooldownChanged* ListenForCooldownChange = NewObject<UCooldownChanged>();
	...
	AbilitySystemComponent->OnActiveGameplayEffectAddedDelegateToSelf.AddUObject(
		ListenForCooldownChange, &UCooldownChanged::OnActiveGameplayEffectAddedCallback);
	...
	for (FGameplayTag CooldownTag : CooldownTagArray)
	{
		AbilitySystemComponent->RegisterGameplayTagEvent(CooldownTag, EGameplayTagEventType::NewOrRemoved).AddUObject(
			ListenForCooldownChange, &UCooldownChanged::CooldownTagChanged);
	}

	return ListenForCooldownChange;
}

void UCooldownChanged::EndTask()
{
	if (IsValid(ASC))
	{
...
void UCooldownChanged::OnActiveGameplayEffectAddedCallback(UAbilitySystemComponent* Target,
...
void UCooldownChanged::CooldownTagChanged(const FGameplayTag CooldownTag, int32 NewCount)
{
	if (NewCount == 0)
	{
...
bool UCooldownChanged::GetCooldownRemainingForTag(FGameplayTagContainer InCooldownTags, float& TimeRemaining,
